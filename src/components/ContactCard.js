const ContactCard = () => {
    return(
        <div className="card lg:card-side bg-base-100 shadow-xl full-w">
 
  <div className="card-body full-w">
  <div>
  <div className="justify-content-center">
        <h1 className="text-4xl font-bold mb-16">Contact me</h1>
        <div >
        <h1 className="text-2xl"><b>Introduction</b></h1>
        <p className="mt-5">Get in contact with me.</p>
        </div>
        <div className="mt-12">
        <h1 className="text-2xl"><b>Email</b></h1>
        <p className="mt-5">N00211885@iadt.ie</p>
        <p className="mt-5">benhackettdelaney9723@gmail.com</p>
        </div>
        <div className="mt-12">
        <h1 className="text-2xl"><b>Socail media links</b></h1>
        <p className="mt-5">Instagram: dont have one</p>
        <p className="mt-5">Facebook: dont have one</p>
        <p className="mt-5">Whatsapp: 0882324323</p>
        </div>
        </div>
        </div>
    </div>
  </div>


    )
}

export default ContactCard;